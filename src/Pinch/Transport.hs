...
data Transport
  ...
  , readMessage  :: forall a . G.Get a -> IO (ReadResult (a, HeaderData))
  }
...
writeVarString bs = B.word32BE (fromIntegral $ BS.length bs) <> B.byteString bs
...
readHeaderMsg :: forall a conn. Connection conn => conn -> IORef BS.ByteString -> IORef Mode -> G.Get a -> IO (ReadResult (a, HeaderData))
readHeaderMsg conn buf modeRef p = do
  ...
  frameParser = do
    ...
    first8Bytes <- G.getWord32be
    if
      | isBinaryProtocol first8Bytes -> (,MUnframed PBinary) <$> p
      | isCompactProtocol first8Bytes -> (,MUnframed PCompact) <$> p
      | otherwise -> do
        -- All other options start with a frame size.
        let frameSize = first8Bytes
        G.isolateLazy (fromIntegral frameSize) $ do
          ...
          second8Bytes <- G.lookAhead G.getWord32be
          if
            | isBinaryProtocol second8Bytes -> (,MFramed PBinary) <$> p
            | isCompactProtocol second8Bytes -> (,MFramed PCompact) <$> p
            | headerMagicMatches second8Bytes -> do
                ...
                when (position > headerEnd) $ fail "Internal error reading header"
                when (position < headerEnd) $ G.skip $ headerEnd - position
                ...
                case phTransforms of
                  ...
                  transforms -> do
                    ...
                    bytes <- G.getByteString $ fromIntegral frameSize - fromIntegral phSize - 14
                    case G.runGet p $ transform bytes of
                      Left err -> fail err
                      Right a -> pure $ addMode a
            | otherwise -> fail "Could not detect client transport type"
...
readVarInt headerEnd = do
  ...
  go (min limit 5) 0 0
  where
  go limit _ _ | limit == 0 = fail "VarInt exceeds bounds of header"
...
readHeader frameSize = do
  ...
  let limitedVarInt = readVarInt headerEnd
  phProtocolId :: Word16 <- limitedVarInt
  numTransforms :: Word16 <- limitedVarInt
  -- The spec (if you can call it that), says transforms can carry data, but
  -- none of the other clients implement this.
  transformIds :: [Word32] <- replicateM (fromIntegral numTransforms) (readVarInt headerEnd)
...
varString headerEnd = do
  n <- readVarInt =<< getLimit
  limit <- getLimit
  when (limit < n) $ fail "Info value exceeds bounds of header"
  G.getByteString n

  where
  getLimit = (headerEnd -) <$> G.bytesRead
  ...
  go' = do
    ...
    case infoId of
      ...
      _ ->
        ...
        -- how much to skiup to get to the next field.
